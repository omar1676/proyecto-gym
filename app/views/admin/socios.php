...
require_once __DIR__ . '/../../helpers/AccessPolicyPresentation.php';
        ...
        <section class="mx-auto max-w-6xl">
            ...
            <section class="mt-6 rounded-[22px] border border-[#e4e4e7] bg-white p-6 shadow-sm" aria-label="Detalle económico del socio">
                ...
                <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="rounded-xl bg-[#f4f4f5] p-4"><p class="text-xs font-bold uppercase text-neutral-500">Acceso lógico</p><p class="mt-1 font-bold text-neutral-800"><?= htmlspecialchars(AccessPolicyPresentation::state($accesoFicha['policy_state'] ?? $accesoFicha['estado']), ENT_QUOTES, 'UTF-8') ?></p><p class="text-xs text-neutral-500"><?= htmlspecialchars(AccessPolicyPresentation::reason($accesoFicha['reason_code'] ?? $accesoFicha['motivo']), ENT_QUOTES, 'UTF-8') ?></p></div>
                </div>
                <?php if (Authorization::can((string)($_SESSION['usuario_rol'] ?? ''), 'training.view')): ?>
                <section class="mt-5 rounded-2xl border border-indigo-200 bg-indigo-50 p-4" aria-label="Entrenamiento del socio">
                    <div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-xs font-extrabold uppercase tracking-wide text-indigo-700">Entrenamiento</p><?php if($trainingSummary):?><p class="mt-1 font-extrabold text-neutral-900"><?=htmlspecialchars($trainingSummary['name'],ENT_QUOTES,'UTF-8')?></p><p class="mt-1 text-sm text-neutral-600"><?=htmlspecialchars($trainingSummary['disciplines']??'GENERAL',ENT_QUOTES,'UTF-8')?> · <?=(int)$trainingSummary['day_count']?> días · desde <?=date('d/m/Y',strtotime($trainingSummary['start_date']))?></p><?php else:?><p class="mt-1 text-sm text-neutral-600">Sin plan principal asignado.</p><?php endif;?></div><?php if($trainingSummary):?><a href="<?=APP_URL?>/index.php?action=training_plans&amp;plan=<?=(int)$trainingSummary['id_training_plan']?>" class="inline-flex min-h-11 items-center rounded-full bg-indigo-600 px-5 text-sm font-bold text-white">Ver plan</a><?php endif;?></div>
                </section>
                <?php endif; ?>
                <?php if (Authorization::can((string)($_SESSION['usuario_rol'] ?? ''), 'retention.view')): ?>
                <section class="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4" aria-label="Seguimiento de asistencia del socio">
                    <div class="flex flex-wrap items-center justify-between gap-3"><div><p class="text-xs font-extrabold uppercase tracking-wide text-amber-800">Atención al socio</p><?php if($retentionSummary):?><p class="mt-1 font-extrabold text-neutral-900"><?=htmlspecialchars($retentionSummary['state_label'],ENT_QUOTES,'UTF-8')?></p><p class="mt-1 text-sm text-neutral-600"><?=htmlspecialchars($retentionSummary['explanation'],ENT_QUOTES,'UTF-8')?> · <?=htmlspecialchars($retentionSummary['last_attendance_label'],ENT_QUOTES,'UTF-8')?></p><?php else:?><p class="mt-1 text-sm text-neutral-600">Todavía no existe una evaluación de asistencia.</p><?php endif;?></div><a href="<?=APP_URL?>/index.php?action=retention&amp;q=<?=urlencode(trim($fichaFinanciera['socio']['nombre'].' '.$fichaFinanciera['socio']['apellidos']))?>" class="inline-flex min-h-11 items-center rounded-full bg-amber-700 px-5 text-sm font-bold text-white">Ver en Retention</a></div>
                </section>
                <?php endif; ?>
                <details class="mt-5">
            ...
            <section class="mt-6 rounded-[22px] border border-[#e4e4e7] bg-white p-6 shadow-sm" aria-label="Política de acceso del socio">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    ...
                    <div class="text-right"><span class="inline-flex rounded-full px-3 py-1 text-xs font-extrabold <?= $accesoFicha['estado']==='PERMITIDO'?'bg-green-100 text-green-800':'bg-red-100 text-red-800' ?>"><?= htmlspecialchars(AccessPolicyPresentation::state($accesoFicha['policy_state'] ?? $accesoFicha['estado']),ENT_QUOTES,'UTF-8') ?></span><p class="mt-1 text-xs text-neutral-500"><?= htmlspecialchars(AccessPolicyPresentation::reason($accesoFicha['reason_code']),ENT_QUOTES,'UTF-8') ?></p></div>
                </div>
                <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    <div class="rounded-xl bg-neutral-100 p-3"><p class="text-xs font-bold text-neutral-500">Política</p><p class="font-bold"><?= htmlspecialchars(AccessPolicyPresentation::state($accesoFicha['policy_state'] ?? 'BASE_MEMBRESIA'),ENT_QUOTES,'UTF-8') ?></p></div>
                    <div class="rounded-xl bg-neutral-100 p-3"><p class="text-xs font-bold text-neutral-500">Sincronización proveedor</p><p class="font-bold"><?= htmlspecialchars(AccessPolicyPresentation::syncState($accesoFicha['provider_sync_state'] ?? 'DISABLED'),ENT_QUOTES,'UTF-8') ?></p><p class="text-[11px] text-neutral-500">La integración física está desactivada y no confirma aperturas.</p></div>
                    <div class="rounded-xl bg-neutral-100 p-3"><p class="text-xs font-bold text-neutral-500">Inicio</p><p class="font-bold"><?= htmlspecialchars(AccessPolicyPresentation::dateTime($accesoFicha['starts_at_utc'] ?? null),ENT_QUOTES,'UTF-8') ?></p></div>
                    <div class="rounded-xl bg-neutral-100 p-3"><p class="text-xs font-bold text-neutral-500">Caducidad</p><p class="font-bold"><?= htmlspecialchars(AccessPolicyPresentation::dateTime($accesoFicha['expires_at_utc'] ?? null),ENT_QUOTES,'UTF-8') ?></p></div>
                </div>
                ...
                <form id="access-policy-form" method="POST" action="index.php?action=admin_access_change" class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" onsubmit="return confirmarCambioAcceso(this)">
                    ...
                    <div><label for="access-operation" class="block text-xs font-bold text-neutral-500">Operación</label><select id="access-operation" name="operacion" required class="mt-1 w-full rounded-xl border px-3 py-2 text-sm">
                        <?php if($canTemporary): ?><option value="temporary">Dar acceso de prueba</option><?php if(($accesoFicha['policy_state']??'')==='TEMPORARY'): ?><option value="extend">Extender acceso temporal</option><?php endif; ?><?php endif; ?>
                        <?php if($canSuspend): ?><option value="suspend">Suspender acceso</option><?php endif; ?><?php if($canDeny): ?><option value="deny">Denegar acceso</option><?php endif; ?><?php if($canPermanent): ?><option value="permanent">BLOQUEO PERMANENTE</option><?php endif; ?><?php if($canRestore): ?><option value="restore">Restaurar acceso</option><?php endif; ?>
                    </select></div>
                ...
                <?php endif; ?>
                <?php if($historialAcceso): ?><details class="mt-5"><summary class="cursor-pointer text-sm font-bold text-indigo-700">Historial de acceso (<?= count($historialAcceso) ?>)</summary><ol class="mt-3 divide-y"><?php foreach($historialAcceso as $event): ?><li class="py-2 text-xs"><strong><?= htmlspecialchars(AccessPolicyPresentation::action($event['action']),ENT_QUOTES,'UTF-8') ?></strong> · <?= htmlspecialchars(AccessPolicyPresentation::state($event['previous_state']),ENT_QUOTES,'UTF-8') ?> → <?= htmlspecialchars(AccessPolicyPresentation::state($event['new_state']),ENT_QUOTES,'UTF-8') ?> · <?= htmlspecialchars(AccessPolicyPresentation::dateTime($event['created_at_utc']),ENT_QUOTES,'UTF-8') ?></li><?php endforeach; ?></ol></details><?php endif; ?>
            </section>
            <?php endif; ?>
...
function fijarTemporal(dias) {
    ...
    if (aviso) aviso.textContent = 'Este acceso se denegará automáticamente el ' + fin.toLocaleString('es-ES') + '.';
}
