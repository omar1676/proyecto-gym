...
$db->exec("DELETE vl FROM venta_linea vl INNER JOIN venta v ON v.id_venta=vl.id_venta INNER JOIN gimnasio g ON g.id_gimnasio=v.id_gimnasio WHERE g.nombre LIKE 'TEST TENANT %'");
$db->exec("DELETE v FROM venta v INNER JOIN gimnasio g ON g.id_gimnasio=v.id_gimnasio WHERE g.nombre LIKE 'TEST TENANT %'");
$db->exec("DELETE FROM venta_linea WHERE id_venta IN (SELECT id_venta FROM venta WHERE serie = 'T')");
$db->exec("DELETE FROM venta WHERE serie = 'T'");
